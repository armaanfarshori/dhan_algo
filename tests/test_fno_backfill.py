...
def test_nifty_atm_strike_rounds_to_step():
    ...
    assert fb.nifty_atm_strike(23425, 50) == 23400  # banker's rounding (round-half-even)


def test_normalize_iv_percent_vs_fraction():
    assert fb._normalize_iv(13.5) == pytest.approx(0.135)   # percent → fraction
    assert fb._normalize_iv(0.135) == pytest.approx(0.135)  # already a fraction
